...
export const useVehicleForm = (editingVehicle?: any) => {
  ...
  const [formData, setFormData] = useState<VehicleFormData>({
    ...
    year: editingVehicle?.year?.toString() || '0',
    model: editingVehicle?.model || '',
    plate: editingVehicle?.miles?.toString() || editingVehicle?.plate?.toString() || '0',
    internalCode: editingVehicle?.internal_code || editingVehicle?.internalCode || '',
    color: editingVehicle?.color || '',
    caNote: editingVehicle?.ca_note?.toString() || editingVehicle?.caNote?.toString() || '0',
    titleInfo: getTitleInfo(editingVehicle),
    purchasePrice: editingVehicle?.purchase_price?.toString() || editingVehicle?.purchasePrice?.toString() || '0',
    salePrice: editingVehicle?.sale_price?.toString() || editingVehicle?.salePrice?.toString() || '0',
