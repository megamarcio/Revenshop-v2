...
const VehiclePhotoDisplay: React.FC<VehiclePhotoDisplayProps> = ({ 
  ...
  // Garantir que a URL esteja correta - CORRIGIDO: vehicle-photos-new
  let finalPhotoUrl = photoUrl;
  
  // Se a URL já está completa, usar como está
  if (!photoUrl.startsWith('http')) {
    // Se não tem http, construir URL completa com o nome correto do bucket
    const cleanPath = photoUrl.replace(/^(vehicle-photos-new\/|vehicle-cards\/)/, '');
    finalPhotoUrl = `https://ctdajbfmgmkhqueskjvk.supabase.co/storage/v1/object/public/vehicle-photos-new/${cleanPath}`;
