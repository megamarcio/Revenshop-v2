...
const ReservationTableRow: React.FC<ReservationTableRowProps> = ({
  ...
  const whatsappUrl = `http://wa.me/${cleanPhone}`;

  // URLs for the action buttons
  const reservationUrl = `https://r3-rental.us5.hqrentals.app/car-rental/reservations/step7?id=${id}`;
  const kommoUrl = kommoLeadId ? `https://r3rentalcar.kommo.com/leads/detail/${kommoLeadId}` : null;
        ...
        <div className="flex items-center gap-1">
          ...
          {kommoUrl && (
            <Button
              ...
              size="sm"
              className="h-6 w-6 p-0"
              onClick={() => window.open(kommoUrl, '_blank')}
