-- Script para executar migração da tabela import_data manualmente
...
DROP FUNCTION IF EXISTS update_import_data_updated_at() CASCADE;
...
DROP TRIGGER IF EXISTS import_data_updated_at ON public.import_data;
...
DROP FUNCTION IF EXISTS public.save_ai_settings(text, text, text, text, text, text, text, text);
